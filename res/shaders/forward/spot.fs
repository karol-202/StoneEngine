...
void main()
{
	...
	//vec3 normal = normalize((texture2D(normalMap, uv).rgb * 2.0 - 1.0) * normalMapIntensity);
	...
	float diffuseFactor = clamp(dot(normalSurf, -lightDirection), 0, 1);
	...
	vec3 reflection = reflect(lightDirection, normalSurf);
